    ...
    <article className="group grid min-h-full grid-rows-[auto_1fr] overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-xl">
      <div className="relative aspect-[4/5] overflow-hidden bg-zinc-100">
        ...
        <div className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-zinc-900 shadow-sm backdrop-blur">
          {product.inventory <= 10 ? "Low stock" : "Canonical stock"}
        </div>
      </div>
      ...
      <div className="grid gap-4 p-4">
        <div className="grid gap-2">
          ...
          <h2 className="text-balance text-lg font-semibold leading-tight text-zinc-950">
            <Link href={`/products/${encodeURIComponent(product.id)}`} className="underline-offset-4 hover:underline">{product.name}</Link>
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-zinc-600">
            ...
            <span className="rounded-full bg-emerald-50 px-2 py-1 text-emerald-700">JPY ledger</span>
          </div>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-3">
          ...
          {quantity === 0 ? (
            <button
              ...
              className="inline-flex h-11 items-center gap-2 rounded-md bg-zinc-950 px-4 text-sm font-semibold text-white transition hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-zinc-300"
            >
